"""
Скромный AI-агент в Телеграме (без прокси, через прямой IP)
Работает в РФ, Python 3.13+, GigaChat API + SQLite
"""
...
# Прямой IP Telegram API (работает без прокси в РФ)
TG_API_SERVER = "https://api.telegram.org"

# ---------- ИНИЦИАЛИЗАЦИЯ ----------
# Создаём сессию с увеличенным таймаутом
session = AiohttpSession()
session.timeout = 60  # 60 секунд на запрос

bot = Bot(
    token=TELEGRAM_BOT_TOKEN,
    session=session,
    base_url=TG_API_SERVER
)
...
# ---------- AI-ФУНКЦИИ ----------
SYSTEM_PROMPT = (
    "Ты — скромный, дружелюбный AI-помощник. Отвечай коротко, по-простому, "
    "как друг. Используй разговорный стиль: 'че случилось', 'ща гляну', 'могу помочь'. "
    "Не будь навязчивым."
)

async def ask_giga(prompt):
...
async def main():
    ...
    # Пробуем удалить вебхук несколько раз
    for attempt in range(5):
        try:
            await bot.delete_webhook(drop_pending_updates=True)
            print("✅ Вебхук удалён, запускаю бота...")
            break
